...
class PerDcompAttachedFile(models.Model):
    """
    Modelo para arquivos anexados aos PER/DCOMPs.
    """
    ...
    class TipoArquivo(models.TextChoices):
        PERDCOMP = "PERDCOMP", "PER/DCOMP"
        AVISO_RECEBIMENTO = "AVISO_RECEBIMENTO", "Aviso de Recebimento"
    ...
    tipo_arquivo = models.CharField(
        max_length=20, choices=TipoArquivo.choices, help_text="Tipo do arquivo anexado"
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["public_id"]),
            models.Index(fields=["perdcomp_id"]),
    ...
    @property
    def perdcomp(self):
        """Propriedade para acessar o PER/DCOMP relacionado."""
        if self.perdcomp_id:
            try:
                return PerDcomp.objects.get(
                    id=self.perdcomp_id, deleted_at__isnull=True
                )
    ...
    def __str__(self):
        perdcomp_numero = (
            self.perdcomp.numero_perdcomp if self.perdcomp else "Desconhecido"
        )
        return (
            f"{self.get_tipo_arquivo_display()} - {self.file_name} ({perdcomp_numero})"
        )


class PerDcompAnnotation(models.Model):
    """
    Modelo para anotações de PER/DCOMPs feitas por usuários.
    """
    ...
    def __str__(self):
        ...
        return f"Anotação de {user_name} para PER/DCOMP {perdcomp_numero}"
